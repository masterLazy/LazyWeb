...
lazy::Web::Web() { set_recv_path("recv/"); }
lazy::Web::~Web() {
...
void lazy::Web::recv_loop(Web& web) {
	...
	while (!web.sg_end) {
		...
		// Recv complete
		if (recving == true &&
			(clock() - timer > WEB_RECV_OVERTIME || !web.sg_recv)) {
			recving = false;
			of.close();
...
bool lazy::Web::load_def_ca(SSL_CTX* ctx) {
...
#ifdef _DEBUG
		cout
			<< "Error: Failed to load CA certificate: CertOpenSystemStore() failed."
			<< endl;
#endif
		return false;
	}
	...
	while ((cert = CertEnumCertificatesInStore(hStore, cert)) != NULL) {
		//.cer->.pem
		X509* x509 = d2i_X509(NULL, (const unsigned char**)&cert->pbCertEncoded,
			cert->cbCertEncoded);
...
bool lazy::Web::check_par_ok(WebProt p, HttpVer v) {
	if ((p == WebProt::http || p == WebProt::https) && v == HttpVer::http_3) {
		return false;
	}
...
#ifdef _DEBUG
		std::cout << "Failed to set recv path: failed to create directory"
			<< std::endl;
#endif
		return false;
	}
...
bool lazy::Web::init_winsock_c() {
	...
	if (sock == -1) {
#ifdef _DEBUG
		std::cout << "Error: Failed to create SOCKET: " << get_err_str() << "."
			<< std::endl;
#endif
		close();
		return false;
...
bool lazy::Web::init(lazy::WebProt p, lazy::HttpVer v, bool _verify) {
	...
	if (!check_par_ok(p, v)) {
#ifdef _DEBUG
		cout << "Error: Failed to initialize: Web protocol does not match HTTP "
			"version."
			<< endl;
#endif
		return false;
...
bool lazy::Web::connect(std::string hostname, int port, float waitSec) {
	...
	if (prot == WebProt::https_quic) {
		...
		if (SSL_set_alpn_protos(ssl, (unsigned char*)alpn.c_str(), alpn.size()) !=
			0) {
...
bool lazy::Web::connect(std::string url, float waitSec) {
	return connect(WebHelper::get_url_host(url), WebHelper::get_url_port(url),
		waitSec);
}
...
bool lazy::Web::init_winsock_s(std::string hostname, int port) {
	...
	if (bind(svr_sock, addr->ai_addr, addr->ai_addrlen) == -1) {
		close();
#ifdef _DEBUG
		std::cout << "Error: Failed to bind SOCKET: " << get_err_str() << "."
			<< std::endl;
#endif
		return false;
	}
...
bool lazy::Web::init(std::string ip, int port, lazy::WebProt p, lazy::HttpVer v,
	std::string cert, std::string key) {
	using namespace std;
	// If parameters are valid
	if (!check_par_ok(p, v)) {
#ifdef _DEBUG
		cout << "Error: Failed to initialize: Web protocol doesn't match HTTP "
			"version."
			<< endl;
#endif
		return false;
...
#ifdef _DEBUG
		cout
			<< "Error: Failed to initialize: Server doesn't support QUIC or HTTP 3."
			<< endl;
#endif
		return false;
	}
	...
	if (p == WebProt::https) {
		...
		if (verify) {
			if (SSL_CTX_use_certificate_file(ctx, cert.c_str(), SSL_FILETYPE_PEM) !=
				1) {
#ifdef _DEBUG
				cout << "Error: Failed to initialize: Failed to set certificate file."
					<< endl;
#endif
				return false;
			}
			if (SSL_CTX_use_PrivateKey_file(ctx, key.c_str(), SSL_FILETYPE_PEM) !=
				1) {
#ifdef _DEBUG
				cout << "Error: Failed to initialize: Failed to set private key file."
					<< endl;
#endif
				return false;
			}
...
#ifdef _DEBUG
				cout << "Error: Failed to initialize: Failed to check certificate file "
...
#ifdef _DEBUG
		cout << "Error: Failed to listen. " << get_err_str() << "." << endl;
#endif
		return false;
	}
	return true;
...
bool lazy::Web::accept() {
	...
	if (sock == -1) {
#ifdef _DEBUG
		std::cout << "Error: Failed to accept connection. " << get_err_str()
			<< std::endl;
#endif
		return false;
	}
...
#ifdef _DEBUG
		std::cout << "Error: Failed to accept connection: getaddrinfo() failed."
			<< std::endl;
#endif
		close_client();
		return false;
...
#ifdef _DEBUG
		std::cout
			<< "Error: Failed to close client connection: Failed to get addrinfo. "
	...
	if (ssl != nullptr) {
		if (SSL_get_state(ssl) != TLS_ST_OK) {
			...
			cout << "Error: Failed to write: SSL state error. The SSL state is \""
				<< SSL_state_string_long(ssl) << "\"(" << SSL_get_state(ssl) << ")"
				<< "." << endl;
#endif
			return false;
		}
	...
	if (ssl != nullptr) {
		...
		while (true) {
			...
			case SSL_ERROR_SYSCALL:
				if (get_err() == WSAEWOULDBLOCK || get_err() == WSAEALREADY)
					continue;
#ifdef _DEBUG
			...
			default:
#ifdef _DEBUG
				cout << "Error: Failed to write: " << get_ssl_err_str() << "." << endl;
#endif
				return false;
			}
		}
...
bool lazy::Web::write(lazy::MsgMaker& msg) { return write(msg.make()); }

bool lazy::Web::msg_empty() { return msg_queue.empty(); }
...
std::string lazy::Web::get_hostname() { return host; }
SOCKET lazy::Web::get_socket() { return sock; }
std::string lazy::Web::get_ipv4() {
	...
	inet_ntop(info->ai_family, &((sockaddr_in*)info->ai_addr)->sin_addr, str,
		INET_ADDRSTRLEN);
	return str;
}
int lazy::Web::get_err() { return WSAGetLastError(); }
std::string lazy::Web::get_err_str() {
	...
	DWORD result = FormatMessageA(
		...
		NULL, err, MAKELANGID(LANG_ENGLISH, SUBLANG_DEFAULT), (LPSTR)&temp, 0,
		NULL);
...
std::string lazy::Web::get_ssl_err_str() {
	...
	std::string str = s;
	if (str == "ca md too weak")
		str = "CA MD too weak";
	else if (str == "ee key too small")
		str = "EE key too small";
	else if (str == "nested asn1 error")
		str = "Nested ASN1 error";
	else
		str[0] = std::toupper(str[0]);
	str += "(" + std::to_string(err) + ")";
	return str;
}
lazy::Web::Mode lazy::Web::get_mode() { return mode; }
lazy::HttpVer lazy::Web::get_http_ver() { return httpv; }
lazy::WebProt lazy::Web::get_protocol() { return prot; }
...
bool lazy::Msg::analyse() {
	...
	while (true) {
		...
		string value = msg.substr(msg.find(": ", i) + 2,
			msg.find("\r\n", i) - (msg.find(": ", i) + 2));
	...
	if (fline.find("GET") != string::npos) {
		if (fline.find("?") != string::npos) {
			pars = fline.substr(fline.find("?") + 1,
				fline.find(" HTTP") - fline.find("?") - 1);
		}
	}
	if (!pars.empty()) {
		i = 0;
		while (pars.find("&", i) != string::npos) {
			par.push_back({
				...
				WebHelper::uri_decode(
					pars.substr(pars.find("=", i) + 1,
								pars.find("&", i) - pars.find("=", i) - 1)),
				});
			i = pars.find("&", i) + 1;
		}
		par.push_back({
	...
	if (get_header("Content-Type").find("multipart/form-data") != string::npos) {
		...
		if (msg.size() - i != 0) {
			...
			string fname = filename.substr(0, filename.find_last_of('/') + 1) +
				"msg_body" +
				WebHelper::get_file_suf(get_header("Content-Type"));
			ofstream of;
...
bool lazy::Msg::analyse_form_data() {
	...
	while (true) {
		...
		string fname = filename.substr(0, filename.find_last_of('/') + 1) +
			"form-data_" + to_string(num) + ".dat";
		ofstream of;
		of.open(fname, ios::binary);
		if (!of.is_open()) {
#ifdef _DEBUG
			cout << "Error: Failed to save msg form-data: Failed to open file."
				<< endl;
#endif
			return false;
		}
		...
		if (iType != string::npos) {
			suf = WebHelper::get_file_suf(
				msg.substr(msg.find(":", iType) + 2,
					msg.find("\r\n", iType) - msg.find(":", iType) - 2));
		}
		// Save
		fname = filename.substr(0, filename.find_last_of('/') + 1) + "form-data_" +
			to_string(num) + suf;
		of.open(fname, ios::binary);
		if (!of.is_open()) {
#ifdef _DEBUG
...
bool lazy::Msg::del_file() { return DeleteFileA(filename.c_str()); }
...
std::string lazy::Msg::get_fline() { return fline; }
...
std::string lazy::Msg::get_req(std::string par) {
	...
	if (fline.empty())
		return "";
	if (fline.find(" ") == string::npos || fline.find(" HTTP") == string::npos) {
		return "";
	}
	if (fline.find("?") == string::npos) {
		return fline.substr(fline.find(" ") + 1,
			fline.find(" HTTP") - (fline.find(" ") + 1));
	}
	else {
		return fline.substr(fline.find(" ") + 1,
			fline.find("?") - (fline.find(" ") + 1));
	}
}
...
std::string lazy::Msg::get_body() {
	...
	ifstream f;
	f.open(filename.substr(0, filename.find_last_of('/') + 1) + "body" +
		WebHelper::get_file_suf(get_header("Content-Type")),
		ios::binary);
	if (!f.is_open()) {
#ifdef _DEBUG
...
lazy::MsgMaker::MsgMaker(int hv) { httpv = hv; }
...
void lazy::MsgMaker::set_request_line(std::string res, std::string method) {
	...
	if (httpv == (int)HttpVer::http_1_1)
		fline += " HTTP/1.1";
	else if (httpv == (int)HttpVer::http_1_0)
		fline += " HTTP/1.0";
}
void lazy::MsgMaker::set_state_line(int state) {
	...
	case 200:
		...
		// 重定向
	case 301:
		fline += "301 Moved Permanently";
		break;
	case 302:
		fline += "302 Found";
		break;
	case 304:
		fline += "304 Not Modified";
		break;
	case 307:
		fline += "307 Temporary Redirect";
		break;
		// 客户端错误
	case 400:
		fline += "400 Bad Request";
		break;
	case 401:
		fline += "401 Unauthorized";
		break;
	case 403:
		fline += "403 Forbidden";
		break;
	case 404:
		...
		// 服务器错误
	case 500:
		fline += "500 Internal Server Error";
		break;
	case 503:
		fline += "503 Service Unavailable";
		break;
	default:
		fline += "500 Internal Server Error";
		break;
...
std::string lazy::MsgMaker::set_header(std::string key) {
	...
	if (key == "Connection") {
		...
		value = WebHelper::get_file_mime(file);
	}
...
#ifdef _DEBUG
		std::cout << "Warning: Failed to set header automatically: Unsupported key."
			<< std::endl;
#endif
		return "";
	}
...
void lazy::MsgMaker::set_body(std::string str) { body = str; }
...
lazy::WebHelper::WebHelper(lazy::Web& _web) { web = &_web; }
...
int lazy::WebHelper::get_url_port(std::string url) {
	...
	if (url.find(":", res) != string::npos) {
		if (url.find("/", res) != string::npos) {
			return atoi(url.substr(url.find(":", res) + 1,
				url.find("/", res) - url.find(":", res))
				.c_str());
		}
		else {
...
std::vector<std::string> lazy::WebHelper::find_url(Msg msg) {
	if (msg.is_html())
		return find_url(msg.get_body());
	else
		return std::vector<std::string>();
}

std::string lazy::WebHelper::get_file_mime(std::string filename) {
	using namespace std;
...
std::string lazy::WebHelper::get_file_suf(std::string filemime) {
	using namespace std;
	// text
	if (filemime.find("text/html") != string::npos)
		return ".html";
	else if (filemime == "text/plain")
		return ".txt";
	else if (filemime == "text/css")
		return ".css";
	else if (filemime == "text/javascript")
		return ".js";
	else if (filemime == "application/json")
		return ".json";
	// image
	else if (filemime == "image/jpeg")
		return ".jpg";
	else if (filemime == "image/png")
		return ".png";
	else if (filemime == "image/gif")
		return ".gif";
	else if (filemime == "image/x-ico")
		return ".ico";
	else if (filemime == "image/webp")
		return ".webp";
	// audio
	else if (filemime == "audio/mp3")
		return ".mp3";
	else if (filemime == "audio/wav")
		return ".wav";
	// video
	else if (filemime == "video/mpeg4")
		return ".mpeg4";
	else if (filemime == "video/avi")
		return ".avi";
	// zips
	else if (filemime == "application/x-zip-compressed")
		return ".zip";
	else if (filemime == "application/x-7z-compressed")
		return ".7z";
	else
		return "";
}
